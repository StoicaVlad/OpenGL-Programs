...
vec3 SpotLight()
{
    ...
    if (spot_light > cos(cut_off)) {
        ...
        vec3 multiplication = light_att_factor * light_intensity * PhongLight() * ShadowFactor();
        vec3 result = multiplication + KA;
...
float VolumetricIllumination()
{
    ...
    for (int i = 0; i < 100; ++i) {
        point_position = eye_position + (ray_direction / 100 * i);
