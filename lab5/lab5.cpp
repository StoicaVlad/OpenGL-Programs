...
void Lab5::Init()
{
    ...
    CreateShader("BonusShader",
        "src/lab/lab5/shaders/BonusShader.VS.glsl",
        "src/lab/lab5/shaders/BonusShader.FS.glsl"
    );
}
...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderMesh(meshes["sphere"], shaders["BonusShader"], model, GetSceneCamera(), viewport_space);
    }
    ...
    const gfxc::Camera *camera, const transform2D::ViewportSpace &viewport_space)
{
    ...
    int offsetLocation = glGetUniformLocation(shader->GetProgramID(), "Offset");
    glUniform1f(offsetLocation, Engine::GetElapsedTime());

    int randomValue = glGetUniformLocation(shader->GetProgramID(), "Random");
    glUniform1f(randomValue, rand() % 2 + 1);
