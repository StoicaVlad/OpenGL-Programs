...
uniform vec3 point_light_positions[2];
uniform vec3 point_light_colors[2];
uniform vec3 spot_light_positions[26];
uniform vec3 spot_light_colors[26];
uniform vec3 spot_light_directions[26];
uniform float spot_light_angles[26];
...
vec3 ComputePhongIllumination(vec3 light_position, vec3 light_color)
{
    ...
    {
        specular_component = material_ks * light_color * pow(max(dot(V,R), 0), material_shininess);
    }
